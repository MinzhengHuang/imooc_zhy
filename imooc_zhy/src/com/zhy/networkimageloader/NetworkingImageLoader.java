...
import com.zhy.networkimageloader.ImageSizeUtil.ImageSize;
 ...
 * @author zhy
 */
public class NetworkingImageLoader {
    private static NetworkingImageLoader mInstance;
    ...
    private NetworkingImageLoader(int threadCount, Type type) {
    ...
    public static NetworkingImageLoader getInstance() {
        if (mInstance == null) {
            synchronized (NetworkingImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new NetworkingImageLoader(DEAFULT_THREAD_COUNT, Type.LIFO);
                }
            }
        }
        return mInstance;
    ...
    public static NetworkingImageLoader getInstance(int threadCount, Type type) {
        if (mInstance == null) {
            synchronized (NetworkingImageLoader.class) {
                if (mInstance == null) {
                    mInstance = new NetworkingImageLoader(threadCount, type);
                }
            }
        }
        return mInstance;
    ...
    private Bitmap loadImageFromLocal(final String path, final ImageView iv) {
        Bitmap bm;
        // 加载图片
        // 图片的压缩
        // 1、获得图片需要显示的大小
        ImageSize imageSize = ImageSizeUtil.getImageViewSize(iv);
        // 2、压缩图片
