...
public class ZoomImageView extends ImageView implements OnScaleGestureListener,
		...
		mGestureDetector = new GestureDetector(context,
				new SimpleOnGestureListener() {
					...
					public boolean onDoubleTap(MotionEvent e) {
						...
						float x = e.getX();
						float y = e.getY();
						Log.e("DoubleTap", getScale() + " , " + mInitScale);
	...
	private class AutoScaleRunnable implements Runnable {
		...
		public void run() {
			...
			if (((tmpScale > 1f) && (currentScale < mTargetScale))
				...
				ZoomImageView.this.postDelayed(this, 16);//每隔16毫秒执行一次run方法
			} else {// 设置为目标的缩放比例
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			//与viewpager滑动冲突的处理,+0.01防止浮点操作出现问题(可不要)
...
//					}
	...
	public void onGlobalLayout() {
		if (once) {
			...
			Log.e(TAG, "mInitScale = " + mInitScale);
	...
	private boolean isCanDrag(float dx, float dy) {
		return Math.sqrt((dx * dx) + (dy * dy)) >= mTouchSlop;//sqrt平方根计算
	}
